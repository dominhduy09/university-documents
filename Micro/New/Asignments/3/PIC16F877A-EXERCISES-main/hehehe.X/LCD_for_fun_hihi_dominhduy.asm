...
;PIR interface with pic.c,20 :: 		ADC_Init();
	CALL       _ADC_Init+0
;PIR interface with pic.c,21 :: 		Lcd_Init(); // Initialize LCD
	CALL       _Lcd_Init+0
;PIR interface with pic.c,22 :: 		Lcd_Cmd(_LCD_CLEAR); // Clear display
	MOVLW      1
	MOVWF      FARG_Lcd_Cmd_out_char+0
	CALL       _Lcd_Cmd+0
;PIR interface with pic.c,23 :: 		Lcd_Cmd(_LCD_CURSOR_OFF); // Cursor off
	MOVLW      12
	MOVWF      FARG_Lcd_Cmd_out_char+0
	CALL       _Lcd_Cmd+0
;PIR interface with pic.c,24 :: 		Lcd_Out(1,1,"MOTION DETECTOR" ); // Write text in first row
	MOVLW      1
...
L_main0:
	...
	NOP
	NOP
;PIR interface with pic.c,27 :: 		TRISB.B0=1; // PIN NUMBER 1 IS DECLARED AS A INPUT
	BSF        TRISB+0, 0
;PIR interface with pic.c,28 :: 		PORTB.B0=0;  // initially declare it as zero
	BCF        PORTB+0, 0
;PIR interface with pic.c,30 :: 		TRISD.B0=0; // PIN NUMBER 1 IS DECLARED AS A OUTPUT
	BCF        TRISD+0, 0
;PIR interface with pic.c,31 :: 		PORTD.B0=0;  // initially declare it as zero
	BCF        PORTD+0, 0
